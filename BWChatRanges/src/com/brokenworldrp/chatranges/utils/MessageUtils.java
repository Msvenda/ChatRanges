...
public class MessageUtils {
    ...
    public static void sendRangedMessage(Player player, String message, ChatRange range, Recipients recipients) {
        ...
        sendMessages(player, message, range, recipients, config, formattedMessage);
    ...
    public static void sendRangedEmote(Player player, String message, EmoteRange range, Recipients recipients) {
        ...
        sendMessages(player, message, range.getEmoteRange(), recipients, config, formattedMessage);

    }

    private static void sendMessages(Player player, String message, ChatRange range, Recipients recipients, Config config, BaseComponent formattedMessage) {
        //log message
        if (config.isRecipientNumberLoggingEnabled()) {
            LoggingUtil.logMessage(String.format("%s | (%d, %d, %d)",
                    formattedMessage.toPlainText(),
                    recipients.recipients.size(),
                    recipients.hiddenRecipients.size(),
                    recipients.spies.size()));
        } else {
            LoggingUtil.logMessage(formattedMessage.toPlainText());
        }
        ...
        if(formattedSpyMessage == null){
            LoggingUtil.logWarning("[sendMessages] spy message is null.");
            return;
        }
